...
<div id ="ContextJoinForm" align="center" >
	<h1 class="page-header">신고게시판 목록(Company)</h1>
	...
	<div class="boardWrite">
		...
		<table border="1" summary="" style="width: 60%">
					...
					<tr style="vertical-align:middle; border:1px solid lightgray;">
							...
							<th style="width: 5%; text-align:center;vertical-align:middle; border:1px solid lightgray;">상품 번호</th>
    ...
    <c:choose>
            <c:when test="${fn:length(list) > 0}">
               <c:forEach items="${list }" var="row">
                  <tr>
                     <td align=center>${row.P_COM_NO }</td>
                     <td align=center>${row.MEM_COM_NO }</td>
                      <td align=center>${row.DEAL_NO }</td>
                     <td align=center>${row.P_COM_SUBJECT }</td>
                     <td align=center>${row.P_COM_CONTENT }</td>
                    
                     <td align=center>
                     
                     <c:if test="${row.P_COM_STATUS == 2}">
                           승인</c:if>
                     </td>
   ...
   <script type="text/javascript">
        $(document).ready(function(){
            ...
            function fn_openBoardList(){
                ...
                comSubmit.setUrl("<c:url value='/admin/BlComList.tea' />");
                comSubmit.submit();
            }
